...
retard = (length(h)-1)/2;
SignalFiltre = filter(h, 1, [Suite_diracs zeros(1,retard)]);
SignalFiltre = SignalFiltre(1+retard:end);
...
%Filtrage
z_sans_bruit= filter(hr,1,[SignalFiltre zeros(1,retard)]);
...
TEB_Signal_recu_sans_bruit = bit_errone_sans_bruit/bit_transmis;
fprintf("le TEB du signal sans bruit est: TEB=%0.3e\n",TEB_Signal_recu_sans_bruit)
...
TEB_avec_bruit = zeros(1,length(SNR_dB));
M=2;
for i=1:length(SNR_dB)
    ...
    indices_positifs = find(ze_avec_bruit >= seuil);
    indices_negatifs = find(ze_avec_bruit < seuil);
...
title("Mapping 5 : TEB théorique et TEB calculé");

figure()
plot(diagramme_oeil_sans_bruit)
title("Mapping 5: Diagramme de l'oeil mapping1")
