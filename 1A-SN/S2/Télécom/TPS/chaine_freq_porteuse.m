...
% % Estimer avec un correlogramme non biaise et tracer la densité spectrale du signal
%correl_non_biaise  = xcorr(x,'unbiased');
%correlogramme_non_biaise = abs(fft(correl_non_biaise));
% 
% pwelch(x)
% 
% frequence_correlogramme = linspace(-Fe/2, Fe/2, length(correlogramme_non_biaise));
% 
% figure('Name','corrélogramme non biaisé');
% plot(frequence_correlogramme,fftshift(correlogramme_non_biaise));
% hx = xlabel('fréquence','FontSize',15);
% set(hx,'Interpreter','Latex');
% hy = ylabel('corrélogramme non biaisé','FontSize',15);
