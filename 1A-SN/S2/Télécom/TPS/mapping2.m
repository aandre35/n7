...
%% 
%% Génération du 2ème Mapping :
%%

%% Modulation
%moyenne non nulle, 
%Mise en forme rectangulaire de durée Ts
%filtre de réception de réponse impulsionnelle rectangulaire de durée Ts

SymbolesMapping =randi([0, 1], 1, n);
...
signal_detecte_sans_bruit(indices_negatifs) = 0;
...
TEB_bruit = zeros(length(SNR_dB),1);
M=2;
for i=1:length(SNR_dB)
    ...
    signal_detecte_avec_bruit(indices_negatifs) = 0;
    %Démapping
...
title("TEB théorique et TEB calculé");

figure()
plot(diagramme_oeil_sans_bruit)
title("Mapping 2: diagramme de l'oeil")
