...
/* écrire un message */
void ecrire(char auteur [TAILLE_AUTEUR], char texte [TAILLE_TEXTE]) {
    /*On cherche le dernier message */
    int id_dernier_message = 0;
    for (int i =0; i<NB_LIGNES; i++) {
        if (discussion[i].numero > id_dernier_message) {
            id_dernier_message = discussion[i].numero;
        }
    }
    dernier0 = id_dernier_message;
    int dernier_message = dernier0 % 20;

    /* On écrit le dernier message */
    bzero(discussion[dernier_message].texte, TAILLE_TEXTE);
    bzero(discussion[dernier_message].texte, TAILLE_AUTEUR);
    strcpy(discussion[dernier_message].texte,texte);
    strcpy(discussion[dernier_message].auteur, auteur);
    discussion[dernier_message].numero = dernier0 + 1;
}
...
int main (int argc, char *argv[]) {
    ...
    while(1) {
        ...
        char* position = strchr(m.texte, '\n');
        if (position != NULL) {
            *position = '\0';
        }
        ecrire(m.auteur, m.texte);
        kill(getpid(), SIGUSR1);
    }
