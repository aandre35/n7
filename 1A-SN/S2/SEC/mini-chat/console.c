...
int main (int argc, char *argv[]) {
    ...
    mkfifo(tubeC2S, S_IRUSR|S_IWGRP);
    mkfifo(tubeC2S, S_IRUSR|S_IWGRP);
    ...
    if (strcmp(pseudo,"fin")!=0) { /* "console fin" provoque la terminaison du serveur */
        ...
        while (strcmp(saisie,"au revoir")!=0) {
            ...
            /* si le message est reçu par le serveur */
              if (FD_ISSET(S2C,&readfds)) {
                bzero(buf, TAILLE_RECEPTION);
                nlus = read(S2C,buf, TAILLE_MSG);
                if (strcmp(buf,"fin de la connexion")==0) {
                  close(ecoute);
                  close(S2C);
                  close(C2S);
                  printf("fin client\n");
                ...
                int curseur_ligne = curseur % NB_LIGNES;
                strcpy(discussion[curseur_ligne],buf);
                afficher(curseur);
                curseur++;
              }
            /* si le message est envoyé par l'utilisateur */
              if (FD_ISSET(0,&readfds)) {
                bzero(saisie, TAILLE_SAISIE);
                necrits = read(0,saisie,TAILLE_SAISIE);
                pos_fin_de_ligne = strchr(saisie,'\n');
                if (pos_fin_de_ligne != NULL) {
                  *pos_fin_de_ligne = '\0';
                }
