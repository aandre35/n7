...
if theta_1 <= theta_2
    indices = find( theta_1 <= theta_donnees_bruitees & theta_donnees_bruitees <= theta_2);
end
if theta_1 > theta_2
    indices = find( theta_1 <= theta_donnees_bruitees | theta_donnees_bruitees <= theta_2);
end
...
m=length(indices);    
x_donnees_bruitees = C(1)*ones(m,1) + ...
			rho_donnees_bruitees(indices).*cos(theta_donnees_bruitees(indices));
y_donnees_bruitees = C(2)*ones(m,1) + ...
			rho_donnees_bruitees(indices).*sin(theta_donnees_bruitees(indices));
