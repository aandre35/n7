...
let v = (value_of_expr (expr,mem) env) in
match v with
| (ErrorValue _),_ as res -> res
| (ReferenceValue ref),m2 -> (match (lookforMem ref m2) with 
                                | NotFound -> (ErrorValue (UnknownReferenceError ref)),m2 
                                | Found result -> result,m2)
...
ruleWrite refexpr valexpr mem env = 
  let v = (value_of_expr (valexpr,mem) env) in
  match v with
  | (ErrorValue _),_ as res -> res
  | res,m2 ->  (let r = (value_of_expr (refexpr,m2) env) in
                match r with
                | (ErrorValue _),_ as res -> res
                | (ReferenceValue ad),m3 -> (match (lookforMem ad m3) with 
                                                | NotFound -> (ErrorValue (UnknownReferenceError ad)),m3 
                                                | Found result -> NullValue,((ad,res)::m3))
                | _ -> ((ErrorValue TypeMismatchError),mem))
...
ruleSequence left right mem env = 
    let v = (value_of_expr (left,mem) env) in
    match v with
    | (ErrorValue _),_ as res -> res
    | NullValue,m2 -> ( match (value_of_expr (right,m2) env) with 
                        | (ErrorValue _),_ as res -> res
                        | res,m3 -> res,m3)
    |_ -> (ErrorValue TypeMismatchError),mem
and
...
(* ...............A COMPLETER .......................................*)
  ...
  let addValue = ReferenceValue add in
    (match (lookforMem add m2) with
    | NotFound -> (addValue, ( add, v)::m2)
    | (Found value) -> ( (ErrorValue RuntimeError), m2 )
  )
;;
