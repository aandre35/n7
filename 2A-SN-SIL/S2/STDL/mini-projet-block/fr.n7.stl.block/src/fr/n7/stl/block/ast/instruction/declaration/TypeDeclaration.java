...
public class TypeDeclaration implements Declaration, Instruction {
	...
	public boolean collectAndPartialResolve(HierarchicalScope<Declaration> _scope) {
		if (_scope.contains(this.name)) {
			Logger.error("Identifiant de Type "+ this.name +" déjà pris");
			return false;
		} else {
			boolean _ok = true; //mettre le collect de type
			if (_ok) {
				_scope.register(this);
				return true;
			} else {
				Logger.error("Type "+ this.toString() +" non accepté");
				return false;			
			}
		}
	}
	...
	public boolean completeResolve(HierarchicalScope<Declaration> _scope) {
		return _scope.contains(this.name) && this.type.resolve(_scope);
	}
