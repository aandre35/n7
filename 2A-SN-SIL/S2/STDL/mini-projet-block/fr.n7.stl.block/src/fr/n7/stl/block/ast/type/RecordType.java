...
public class RecordType implements Type, Declaration, Scope<FieldDeclaration> {
	...
	public boolean equalsTo(Type _other) {
		if ((RecordType) _other instanceof RecordType) {
			RecordType _otherRecord = (RecordType) _other;
			if (_other.length() != this.length()) {
				return false;
			} else {
				for (int k=0; k<this.length();k++) {
					if (!this.fields.get(k).equals(_otherRecord.getFields().get(k))) {
						return false;
					} 
				}	
				return true;
	...
	public int getFieldOffset(String _name) {
		...
		FieldDeclaration _current = null;
		Iterator<FieldDeclaration> _iter = this.fields.iterator();
		while (_iter.hasNext() && (! _ok)) {
			_current = _iter.next();
			_ok = _current.getName().contentEquals(_name);
			if (!_ok){
				_offset += _current.getType().length();
			}
		}
		if (_ok) {
			return _offset;
		}
		Logger.error("Le field " + _name + " dans " + this.name + " n'existe pas.");
		return -1;
	}
}
