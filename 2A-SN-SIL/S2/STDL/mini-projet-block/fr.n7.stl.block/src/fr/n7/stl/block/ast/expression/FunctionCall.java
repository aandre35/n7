...
public class FunctionCall implements Expression {
	...
	public boolean collectAndPartialResolve(HierarchicalScope<Declaration> _scope) {
		if (_scope.knows(this.name)) {
			Declaration _function = _scope.get(this.name);
			if (_function instanceof FunctionDeclaration) {
				this.function = ((FunctionDeclaration) _function);
				return true;
			} else {
				Logger.error("La déclaration pour la fonction " + this.name + " est du mauvais type.");
				return false;
			}
		} else {
			Logger.error("L'identifiant " + this.name + " n'a pas été trouvé.");
	        return false;
		}
	}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		code.add(_factory.createCall(this.name, Register.LB));
		return code;
	}
