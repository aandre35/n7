...
public class NamedType implements Type {
	...
	/** 
	 * un NamedType qui désigne respectivement un 
	 * PointerType, ArrayType, RecordType, voire un NamedType lui même désignant ces éléments... 
	 * et ceci récursivement...
	 * @param type
	 * @return
	 */
	public static Type getNamedType(Type type) {
		// Si le NamedType est lui même NamedType
        if (type instanceof NamedType) {
            return getNamedType(((NamedType) type).getType());
        }
        // Si le NamedType est un CoupleType
        else if (type instanceof CoupleType) {
        	Type first = ((CoupleType) type).getFirst();
        	Type second = ((CoupleType) type).getSecond();
        	return new CoupleType(getNamedType(first), getNamedType(second));
        }
        // Autres cas : PointerType, ArrayType, RecordType...
        else {
            return type;
        }
    }
