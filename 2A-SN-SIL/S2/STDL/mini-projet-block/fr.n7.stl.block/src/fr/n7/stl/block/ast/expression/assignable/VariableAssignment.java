...
public class VariableAssignment extends AbstractIdentifier implements AssignableExpression {
	...
	public Fragment getCode(TAMFactory _factory) {
		// Crétation du fragment de code
		Fragment code = _factory.createFragment();
		
		//On empile l'adresse
		Register _register = this.declaration.getRegister();
		int _offset = this.declaration.getOffset();
		int _sizes = this.getType().length();
		TAMInstruction codeAssignment =_factory.createStore(_register, _offset, _sizes);
		code.add(codeAssignment);
